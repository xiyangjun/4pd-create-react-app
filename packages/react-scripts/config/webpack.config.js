...
module.exports = function (webpackEnv) {
  ...
  const isEnvAnalyzer = process.env.BUNDLE_ANALYZER === 'true';
  const DLL_DIR = path.join(paths.dllConfig, isEnvProduction ? 'prod' : 'dev');
  const isDll = isEnvDevelopment ?
    process.env.DEV_DLL === 'true' :
    process.env.PROD_DLL === 'true';
  ...
  const getStyleLoaders = (cssOptions, preProcessor, otherOptions) => {
    ...
    if (preProcessor) {
      // less-loader和resolve-url-loader 有冲突
